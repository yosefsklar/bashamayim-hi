...
export default class Player {
  ...
  update = (lowestBlock, difficulty, blocks, blockOffset, ctx, holdingLeftKey, holdingRightKey) => {
    ...
    for (let i = 0; i < blocks.length; i++) {
      if (this.ySpeed >= 0) {
        if (
          this.x >= blocks[i].x - this.width + U.adjustX(15) &&
          this.x <= blocks[i].x + blocks[i].width - U.adjustX(15) &&
      ...
      if (this.y > blocks[i].y) {
        ...
        if (blocks[i].wordType === "spike") {
          if (
            this.x >= blocks[i].x - this.width + U.adjustX(28) &&
            this.x <= blocks[i].x + blocks[i].width - U.adjustX(28) &&
